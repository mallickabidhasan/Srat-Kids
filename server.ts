...
async function startServer() {
  ...
  app.post("/api/send-admission-form", async (req, res) => {
    ...
    const isEmailConfigured = Boolean(
      ...
      !emailPass.startsWith("<") && 
      emailPass.length >= 8
    );

    if (!isEmailConfigured) {
      // Return success gracefully so student application flow is never disrupted
      return res.json({ 
        success: true, 
        message: "আবেদন সফলভাবে গ্রহণ করা হয়েছে।" 
      });
    }
    ...
    try {
      ...
      console.warn(
        "Notice: Admission notification email was not delivered (Gmail App Password may need updating in Settings):", 
        mailErr?.message || mailErr
      );
      res.json({ success: true, message: "আবেদন সফলভাবে গ্রহণ করা হয়েছে।" });
    }
  });
  ...
  app.post("/api/send-scholarship-form", async (req, res) => {
    ...
    if (!isEmailConfigured) {
      // Application has already been stored in Firestore, return success gracefully
      return res.json({ 
        success: true, 
        message: "বৃত্তি আবেদন ডেটাবেজে সংরক্ষিত হয়েছে।" 
      });
    }
    ...
    try {
      ...
      console.warn(
        "Notice: Admin scholarship email notification was not sent (Gmail App Password may need updating in Settings):",
        mailErr?.message || mailErr
      );
      res.json({ success: true, message: "বৃত্তি আবেদন সফলভাবে সংরক্ষিত হয়েছে।" });
    }
  });
